    ...
    <div class="memory-preview__body">
      ...
      <div class="instrument-preview-container">
        <img
          ...
          :src="instrumentThumbnail"
          alt="Image de l'instrument"
        />
        <div>
          <p>{{ data.instrument.name }} - {{ data.instrument.brand }}</p>
          <p>de {{ data.instrument.name }} - {{ data.instrument.brand }}</p>
        </div>
      </div>
    </div>
  </component>
...
export default {
  ...
  computed: {
    linkUrl() {
      console.log(this.data.instrument);
      return `/instrument/${this.data.instrument.id}/souvenir/${this.data.id}`;
    },
    editLinkUrl() {
    ...
    instrumentThumbnail() {
      return this.data.instrument.image[0].path;
    },
    date() {
